...
def run(
    ...
    print("Set max_split_size_mb:128")
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        for epoch in range(epoch_resume, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                dataloader = torch.utils.data.DataLoader(
                    ds, batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=False, drop_last=(phase == "train"))
        ...
        if run_test:
            ...
            for split in ["test"]:
                # 清理验证集后的缓存和模型
                if split == "val" and device.type == "cuda":
                    torch.cuda.empty_cache()
                    gc.collect()

                # 在测试阶段重新加载模型
                if split == "test":
                    model.load_state_dict(torch.load(os.path.join(output, "best.pt"))['state_dict'])
                ...
                with torch.no_grad():
                    ...
                    dataloader = torch.utils.data.DataLoader(
                        echonet.datasets.Echo(root=data_dir, split=split, **kwargs),
                        batch_size=batch_size, num_workers=num_workers, shuffle=True, pin_memory=False)
                    ...
                    dataloader = torch.utils.data.DataLoader(
                        ds, batch_size=1, num_workers=num_workers, shuffle=False, pin_memory=False)
                    ...
                    # plt.axis([lower - 20, upper + 15, lower - 20, upper + 15])
                    plt.xlim(10, 90)  # 调整 x 轴范围
                    plt.ylim(10, 90)  # 调整 y 轴范围
                    ...
                    plt.savefig(os.path.join(output, "{}_scatter.png".format(split)))
                    plt.close(fig)
